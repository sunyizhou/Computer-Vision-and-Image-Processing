...
def detect(img, template):
    ...
    template_edge_x = detect_edges(template, kernel_x, False)
    template_edge_y = detect_edges(template, kernel_y, False)
    template_edges = edge_magnitude(template_edge_x, template_edge_y)

    #print("edge detect finished")

    coordinates = []
    for i in range(0, len(img_edges)-len(template_edges)):
        for j in range(0, len(img_edges[0])-len(template_edges[0])):
            score = 0
            for k in range(0, len(template_edges)):
                for l in range(0, len(template_edges[0])):
                    score += abs(img_edges[i+k][j+l] - template_edges[k][l])
