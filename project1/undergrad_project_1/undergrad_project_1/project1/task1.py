...
def convolve2d(img, kernel):
    ...
    img_conv = []
    for i in range(0, len(pad_img) - 2):
        l = []
        for j in range(0, len(pad_img[i]) - 2):
            patch = utils.crop(pad_img, i, i + 3, j, j + 3)
            # print(len(patch),len(patch[-1]))
            activation = utils.elementwise_mul(patch, kernel)
            l.append(sum(activation[0]) + sum(activation[1]) + sum(activation[2]))
        img_conv.append(l)
...
def normalize(img):
    ...
    for i in range(len(img)):
        l = []
        for j in range(len(img[i])):
            l.append((img[i][j] - min) / (float)(max - min) * 255)
        normalized_img.append(l)
...
def edge_magnitude(edge_x, edge_y):
    ...
    for i in range(len(edge_x)):
        ...
        for j in range(len(edge_x[i])):
            l.append(np.sqrt(pow(edge_x[i][j], 2) + pow(edge_y[i][j], 2)))
        edge_mag.append(l)
...
def main():
    ...
    img = read_image('D:\CSE473\project1\\undergrad_project_1\\undergrad_project_1\project1\data\proj1-task1.jpg')
